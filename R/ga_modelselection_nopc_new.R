ga_modelselection_nopc_new <- function(Y,X,regions,regionsnames,significant,number_cores,maxiterations,runs_til_stop,kinship = FALSE){
  ...
  original_n <- ncol(X)
  Xy <- cbind(Y,X,regions)
  y1 <- c("y",paste0("SNP",1:ncol(X)),paste0("Region",1:ncol(regions)))
  colnames(Xy) <- y1
  y1 <- y1[y1 %in% c(paste0("SNP",1:ncol(X)),paste0("Region",1:ncol(regions)))]
  X <- as.matrix(Xy[,y1])
  Y <- matrix(Xy[,1],ncol = 1)
  regions <- as.matrix(Xy[,colnames(Xy)%in%paste0("Region",1:ncol(regions))])
  rm(Xy)
