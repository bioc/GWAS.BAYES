preselection_pc <- function(Y,X,number_cores,principal_components,frequentist,controlrate,threshold,nullprob,alterprob,kinship = FALSE){
  ...
  if(is.logical(kinship)){
    ...
    if(.Platform$OS.type == "unix"){
      SNPdata_list <- mclapply(SNPdata_list,SNP_data_function_pcp,pcp = principal_components,mc.cores = number_cores,int = 1)
    } else{
      cl <- makeCluster(number_cores)
      clusterExport(cl,c("SNPdata_list","SNP_data_function_pcp","principal_components"),envir=environment())
      SNPdata_list <- parLapply(cl,SNPdata_list,SNP_data_function_pcp,pcp = principal_components,int = 1)
      stopCluster(cl)
    }
    ...
    if(frequentist){
      ...
      #Bayesian SLR
      w <- cbind(1,principal_components)
    ...
    if(.Platform$OS.type == "unix"){
      SNPdata_list <- mclapply(SNPdata_list,SNP_data_function_pcp,mc.cores = number_cores,pcp = principal_components,int = intercept)
    } else{
      cl <- makeCluster(number_cores)
      clusterExport(cl,c("SNP_data_function_pcp"),envir=environment())
      SNPdata_list <- parLapply(cl,SNPdata_list,SNP_data_function_pcp,pcp = principal_components,int = intercept)
      stopCluster(cl)
    }
    ...
    if(frequentist){
      ...
      #Bayesian RE model
      w <- cbind(intercept,principal_components)
      BIC.null <- optim_llik_RE_BIC(w,y = Y,d = D)
