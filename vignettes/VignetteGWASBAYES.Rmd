...
* Pre-screen individual SNPs in a traditional GWAS manor.
...
Indeed the correlation plot indicates that all correlations between pairs of these 3 SNPs are equal to 1. To investigate the further the minor allele frequency of these 3 significant SNPs can be calculated with the following code:

```{r}
1 - colMeans(SNPs[,Significant_SNPs$Significant == 1])
```

The 3 significant all of the same minor allele frequency of 0.04268. We can look at the number of SNPs in between these 3 SNPs to understand the area of the gene.

```{r}
which(Significant_SNPs$Significant == 1)
```

There is 18 SNPs between the first two significant SNPs, 31 SNPs between the second and third significant, with the total number of SNPs being 52. These values are only looking at SNPs used in this analysis and not SNPs that were dropped due to low minor allele frequency. To get a more in depth picture the summary statistics for the minor allele frequency of these 52 SNPs is given below:

```{r}
summary(1 - colMeans(SNPs[,902:953]))
```

There appears to be a high number of minor allele's for these 52 SNPs indicating this is a great location to further investigate in a lab setting.
