...
`GWAS.BAYES` goes one step further. The second stage of `GWAS.BAYES` performs Bayesian model search where the models are composed of the set of SNPs from the screening phase and/or corresponding pseudo-haplotypes. This second stage allows the SNPs to compete with each other, and typically results in a much smaller set of SNPs. This second stage provides more information on which are the most promising SNPs for follow-up lab experiments. 
...
This vignette explores three different examples to show how the functions provided in `GWAS.BAYES` to implement the above points. The three case studies of interest are: 
...
To prepare SNPs for the analysis the data must be taken from its raw state (above) into a finer state to speed up analysis. First, apply the `standardize` function to transform the SNPs to {0.1}. This can be done in two ways. The first way is alphabetically; meaning that if the letter comes first in the alphabet then that would be a 0 while the letter that comes second in the alphabet would be a 1. The second way to do this is by major and minor allele, the major allele becomes 0 and the minor allele becomes a 1. Note either method will lead to the same significant SNPs; the two will differ by the interpretation of the results. The default method is `method = "major-minor"` and the possible methods are `method=c("major-minor","alphabetical")`.
...
This plot highlights the correlations between pairs of SNPs with the diagonal showing correlation between the same SNP. The correlation ranges from -1 to 1. Depending on how many significant SNPs plotted, the correlation plot created from the `cor_plot` function will be different.
...
Similarly to the Bonferroni correction, the pre-screen step for the Bayesian False Discovery Correction suggests the first 5 SNPs are significant.
...
The `qqman` package can also make QQ plots for the p-values with the function `qq`. The function plots the expected value (under the null hypothesis of no SNP effect) of -log10(p-value) in the x-axis and the abserved p-value in the y-axis. Departures from a straight line indicate that there are SNPs that are predictive of the phenotype. Here is how to use the `qq` function:

```{r, fig.wide = TRUE,fig.align="center", fig.width=6.75, fig.height=4.75}
qq(Significant_SNPs$P_values)
```

Because the line deviates from a straight lin, there is evidence that there are SNPs that are predictive of the ratio of average lateral root and main root length at low salt stress levels.

## Bayesian Model Selection

Next we perform Bayesian model selection to identify, amongst these three SNPs, which ones are important for predicting the phenotype.
...
With three candidate SNPs, there are 2^3-1=7 possible models. Because the number of candidate SNPs is 3 (<= 12), the function `postGWAS` performs an exhaustive search and computes the posterior probabilities for all 7 possible models. In this example, all possible models have the same posterior probability equal to 1/7 = 0.1427. One way to interpret this result is that each of these three SNPs provides the same information. Another way of looking at this result is that having 1 SNP vs having all 3 SNPs in the model makes no difference. This may indicate high correlations amongst these 3 SNPs. 
...
Lets investigate this further with a correlation plot.

```{r, fig.wide = TRUE,fig.align="center", fig.width=6.75, fig.height=4.75}
cor_plot(SNPs = SNPs[,Significant_SNPs$Significant == 1],
  significant = GA_results$Solution[7,],
  info = RealDataInfo[,Significant_SNPs$Significant == 1])
```

Indeed the correlation plot indicates that all correlations between pairs of these 3 SNPs are equal to 1. Further investigation reveals that these 3 SNPs had the same minor allele frequency and there was about 25 SNPs between each of these SNPs.
